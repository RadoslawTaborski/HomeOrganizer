...
export class PermanentItemService implements HttpServiceModel {
  ...
  get(id: string, deep?: number): Promise<PermanentItemModel> {
    return this.http
      .get<ResponseData>(this.configService.config.api + Api.PERMANENT_ITEMS_END_POINT + `/${id}`)
      .pipe(
  ...
  update(item: any): Promise<string> {
    return this.http.put(this.configService.config.api + Api.PERMANENT_ITEMS_END_POINT, item, httpOptions).pipe(
      map((resp: { uuid }) => resp.uuid)
    ).toPromise();
  }
