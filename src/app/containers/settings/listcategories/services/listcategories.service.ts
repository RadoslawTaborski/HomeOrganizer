...
export class ListcategoriesService implements HttpServiceModel {
  ...
  fetch(filters?: { [key: string]: any; }): Promise<ResponseData> {
    return this.http.get<ResponseData>(Api.LIST_CATEGORIES_END_POINT, {params: filters}).toPromise();
  }

  get(id: string, deep?: number): Promise<ListCategory> {
    return this.http
      .get<ResponseData>(Api.LIST_CATEGORIES_END_POINT + `/${id}`)
      .pipe(
  ...
  add(item: any): Promise<string> {
    return this.http.post(Api.LIST_CATEGORIES_END_POINT, item, httpOptions).pipe(
      map((resp: { uuid }) => resp.uuid)
    ).toPromise();
  }

  update(item: any): Promise<string> {
    return this.http.put(Api.LIST_CATEGORIES_END_POINT, item, httpOptions).pipe(
      map((resp: { uuid }) => resp.uuid)
    ).toPromise();
  }

  remove(id: string): Promise<any> {
    return this.http.delete(Api.LIST_CATEGORIES_END_POINT+`/${id}`).pipe(
      map((resp: { data }) => resp.data)
    ).toPromise();
  }
}
